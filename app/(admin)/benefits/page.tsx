...
import { deleteBenefitAction } from "@/app/actions/benefit/benefit-actions";
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-zinc-800">Benefit Management</h3>
          <p className="text-sm text-zinc-500">Manage the highlights of your plant rental service.</p>
        </div>
        <Link
          href="/benefits/create"
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-100">
                <th className="px-6 py-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Icon</th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {benefits.map((benefit) => (
                <tr key={benefit.id} className="hover:bg-zinc-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-sm text-zinc-600 line-clamp-1">{benefit.desc}</p>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <DeleteButton id={benefit.id} action={deleteBenefitAction} />
                    </div>
                  </td>
                </tr>
              ))}
              {benefits.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-10 text-center text-zinc-500 italic text-sm">
                    No benefits found. Start by adding one.
                  </td>
                </tr>
              )}
            </tbody>
